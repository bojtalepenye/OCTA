    ...
    def write_matches(self, matches: List[Tuple[str, str, str]], warnings: List[Tuple[str, str, str, str]], source_file: str):
        ...
        if matches:
            for base_file in self.base_files:
                ...
                match_file = os.path.join(matches_dir, f"{base_name}_vs_{source_name}_matches.txt")
        ...
        if warnings:
            for base_file in self.base_files:
                base_name = Path(base_file).stem
                warning_file = os.path.join(warnings_dir, f"{base_name}_vs_{source_name}_warnings.txt")
                with open(warning_file, 'w') as f:
                    f.write(f"# Warnings for {base_name} vs {source_name}\n\n")
